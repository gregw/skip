...
- **Racer - Start Line Visualization** – A full-frame drawing of the start line with your boat against it at true scale, the start zone it sits in, and the approach the time to line is computed over. An Edit button sets the line ends and switches between named lines. Needs the `signalk-racer` plugin.
